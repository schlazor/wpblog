...
namespace :style do
  ...
  begin
    ...
    desc 'Run Ruby style checks'
    RuboCop::RakeTask.new(:rubycorrect) do |r|
      r.options << '-a'
    end
  rescue LoadError => e
    puts ">>> Gem load error: #{e}, omitting style:rubycorrect" unless ENV['CI']
  end
  ...
  begin
    ...
    desc 'Run Ruby style checks'
    RuboCop::RakeTask.new(:rubyconfig) do |r|
      r.options << '--auto-gen-config'
    end
  rescue LoadError => e
    puts ">>> Gem load error: #{e}, omitting style:rubycorrect" unless ENV['CI']
  end
  ...
  begin
    ...
    desc 'Run Ruby style checks'
    RuboCop::RakeTask.new(:rubycorrectconfig) do |r|
      r.options << '--auto-gen-config'
      r.options << '-a'
    end
  rescue LoadError => e
